...
Además, `Level._buildSurfaceScatter()` siembra automáticamente **matas en el borde superior de las plataformas** (solo visual, sin física), integrándolas con el entorno. La config vive en `Level.SCATTER`, **indexada por tema** — cada bioma declara sobre qué texturas de terreno actúa y qué reparte (`forest` → hierba/flores/setas sobre `grass`; `cave` → musgo/hongos/cristales sobre `stone`; `ruins` → escombros/chatarra/hierba seca sobre `stone`). Indexar por tema (no por textura) evita que un mismo terreno (p. ej. `stone`, usado en cueva, ruinas y nieve) herede la dispersión de otro bioma. Añadir uno nuevo es añadir una entrada, sin tocar la lógica.
